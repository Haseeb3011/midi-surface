...
- **Phase 4 — Customization** (recommended next): drag-and-drop layout editor (dnd-kit), multi-page layouts with on-screen tabs, preset save/load via Dexie + JSON I/O, bundled FL Studio + Ableton template presets, per-control overrides (color, label, icon, MIDI channel, CC/note number)
