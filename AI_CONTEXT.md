...
- **Phase 5 — Quick wins**: hotkeys, perf mode, settings export/import (✅ 2026-05-07)
...
| Date | Change | By |
|---|---|---|
| 2026-05-07 | **Phase 5 — Quick wins.** Hotkeys via `react-hotkeys-hook` (Space → play pulse, R → record pulse, P → perf mode, Esc → exit fullscreen+perf, 1–4 → pad banks A–D; disabled in form elements). Perf mode: `perfMode` bool in `settingsStore` (persist v4); sets `data-perf-mode` on `<html>` imperatively; CSS hides `header`. "perf" button added to header. Settings export/import: JSON file containing settings, MIDI Learn assignments, and active layout; imported layouts get a new ID + `(imported)` suffix. `learnStore.setAssignments()` added for bulk restore. | Claude |
